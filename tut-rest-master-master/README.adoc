...
* `id`, `category`, and `brand` are the attribute for our domain object, the first being marked with more JPA annotations to indicate it's the primary key and automatically populated by the JPA provider.
...
----
...
2018-08-09 11:36:26.169  INFO 74611 --- [main] payroll.LoadDatabase : Preloading Employee(id=1, category=Bilbo Baggins, brand=burglar)
2018-08-09 11:36:26.174  INFO 74611 --- [main] payroll.LoadDatabase : Preloading Employee(id=2, category=Frodo Baggins, brand=thief)
...
----
...
[{"id":1,"category":"Bilbo Baggins","brand":"burglar"},{"id":2,"category":"Frodo Baggins","brand":"thief"}]
----
...
----
{"id":3,"category":"Samwise Gamgee","brand":"gardener"}
----
...
----
{"id":3,"category":"Samwise Gamgee","brand":"ring bearer"}
----
...
This decompressed output shows not only the data elements you saw earlier (`id`, `category` and `brand`), but also a `_links` entry containing two URIs. This entire document is formatted using http://stateless.co/hal_specification.html[HAL].
